    ...
    namespace JSX {
        interface IntrinsicElements {
            'mux-player': React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement> & {
                'playback-id': string;
                // stream-type pode ser "live", "live:dvr" ou "on-demand"
                'stream-type': string; 
                controls: boolean;
                autoplay: boolean;
                muted: boolean;
...
const DroneStreamPlayer: React.FC<{ playbackId: string, droneName: string }> = React.memo(({ playbackId }) => {
    return (
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
            ...
            {/* MUDANÇA CRUCIAL: stream-type alterado para "live:dvr" para habilitar a linha do tempo. 
              Isso permite ao usuário voltar e ver cenas anteriores, transformando o 
              stream ao vivo em um feed com DVR ativado.
            */}
